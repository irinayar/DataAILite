...
Partial Class ScheduledDownloads
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Request("mnth") Is Nothing Then
            ...
            End If
            Session("curmonth") = curmonth & "01"
            Response.Redirect("ScheduledDownloads.aspx")
        End If
        ...
        If Not Request("calndr") Is Nothing AndAlso Request("calndr").ToString.Trim = "add" Then
            ...
            indf = GetScheduledDownloadsIdentifier()
            Session("indf") = indf
            'add future scheduled Downloads
            ...
            'add first scheduled Downloads
            ret = AddScheduledDownloads(url, Session("logon"), rundate, towhom, indf, recc)
        ...
        Dim sql As String = "SELECT ID FROM ourscheduleddownloads ORDER BY ID DESC"
        ...
        sql = "SELECT * FROM ourscheduleddownloads WHERE Deadline LIKE '" & curmonth & "%' AND UserId='" & Session("logon") & "' "
        'End If
        ...
        '    ret = SendEmailScheduledDownloads(AssignmentTable, Session("email"))  'clean from unproper urls as well
        '    If ret <> "Query executed fine." Then
        '        'refresh AssignmentTable
        '        If Session("admin") = "super" Then
        '            sql = "SELECT * FROM ourscheduleddownloads INNER JOIN OURReportInfo ON (ourscheduleddownloads.ReportId=OURReportInfo.ReportId) WHERE Deadline LIKE '" & curmonth & "%' AND ReportDB LIKE '%" & Session("ReportDBforSuper").ToString.Trim.Replace(" ", "%") & "%' "
        '        Else
        '            sql = "SELECT * FROM ourscheduleddownloads WHERE Deadline LIKE '" & curmonth & "%' AND UserId='" & Session("logon") & "' "
        '        End If
        '        If fltr.Trim <> "" Then sql = sql & " AND (" & fltr & ") "
        ...
        Label3.Text = "Scheduled Downloads for " & curmonth.Replace("-", " ").Trim.Replace(" ", "/")
        ...
        If AssignmentTable.Rows.Count > 0 Then
            ...
            For i = 0 To AssignmentTable.Rows.Count - 1
                ...
                ret = AddRowIntoHTMLtableWithNcols(AssignmentTable.Rows(i), SchedDowns, 8)
                ...
                SchedDowns.Rows(i + 4).Cells(6).Style.Item("font-size") = "10px"
                ...
                SchedDowns.Rows(i + 4).Cells(2).InnerHtml = FormatAsHTMLsimple(AssignmentTable.Rows(i)("DEADLINE").ToString & " | " & AssignmentTable.Rows(i)("Prop1").ToString)
                SchedDowns.Rows(i + 4).Cells(3).InnerText = AssignmentTable.Rows(i)("Status").ToString

                SchedDowns.Rows(i + 4).Cells(4).InnerHtml = ""
                SchedDowns.Rows(i + 4).Cells(4).InnerHtml = AssignmentTable.Rows(i)("ToWhom").ToString

                rid = AssignmentTable.Rows(i)("ID").ToString

                SchedDowns.Rows(i + 4).Cells(5).InnerText = ""
                ...
                SchedDowns.Rows(i + 4).Cells(5).Controls.Add(ctl)
                ...
                SchedDowns.Rows(i + 4).Cells(6).Controls.Add(ctrl)

                SchedDowns.Rows(i + 4).Cells(7).VAlign = "top"
                SchedDowns.Rows(i + 4).Cells(7).InnerText = AssignmentTable.Rows(i)("Prop3").ToString
    ...
    Protected Sub btnDelOne_Click(sender As Object, e As EventArgs)
        ...
        Dim sql As String = "DELETE FROM ourscheduleddownloads WHERE ID=" & id & " AND UserId='" & Session("logon") & "'"
        ...
        Else
            MessageBox.Show("Download deleted from the list of Scheduled Downloads", "Delete Schedule", "", Controls_Msgbox.Buttons.OK, Controls_Msgbox.MessageIcon.Information)
        End If
        Response.Redirect("ScheduledDownloads.aspx")
    End Sub
    Protected Sub btnDelAll_Click(sender As Object, e As EventArgs)
        ...
        Dim dt As DataTable = mRecords("SELECT * FROM ourscheduleddownloads WHERE ID = " & id, err).ToTable
        Dim indf As String = dt.Rows(0)("Prop3").ToString
        Dim sql As String = "DELETE FROM ourscheduleddownloads WHERE Prop3='" & indf & "' AND UserId='" & Session("logon") & "'"
        Dim ret As String = String.Empty
        ret = ExequteSQLquery(sql)
        If ret <> "Query executed fine." Then
            ret = "ERROR!! " & ret
            MessageBox.Show(ret, "Delete Schedule", "", Controls_Msgbox.Buttons.OK, Controls_Msgbox.MessageIcon.Error)
        Else
            MessageBox.Show("Reccurence deleted from the list of Scheduled Downloads", "Delete Schedule", "", Controls_Msgbox.Buttons.OK, Controls_Msgbox.MessageIcon.Information)
        End If
        Response.Redirect("ScheduledDownloads.aspx")
    End Sub

    Private Function AddScheduledDownloads(ByVal url As String, ByVal logon As String, ByVal rundate As String, ByVal ToWhom As String, ByVal indf As String, ByVal runnumber As String) As String
        Dim ret As String = String.Empty
        Dim sqlq As String = String.Empty
        Try
            sqlq = "INSERT INTO ourscheduleddownloads (StartDate,[Status],UserId,URL,Deadline,ToWhom,Prop3,Prop1) VALUES ('" & DateToString(Now()) & "','scheduled',"
            sqlq = sqlq & "'" & logon & "',"
            sqlq = sqlq & "'" & url & "',"
            sqlq = sqlq & "'" & rundate & "',"
    ...
    Public Function GetScheduledDownloadsIdentifier() As String
        Dim ret As String = String.Empty
        Dim m As Integer = 0
        ...
        Return ret
    End Function
    Private Function SendEmailScheduledDownloads(ByRef dts As DataTable, ByVal useremail As String) As String
        'NOT IN USE
        Dim ret As String = String.Empty
        ...
        Try
            For i = 0 To dts.Rows.Count - 1
                ...
                If url.Trim.LastIndexOf(".") <= 0 OrElse Not url.ToLower.StartsWith("http") Then
                    WriteToAccessLog(Session("logon"), "Record deleted from ourscheduleddownloads. Format of URL does not supported: " & url & " It should end with .CSV, .XML, .JSON, .TXT, .XLS, .XLSX, .MDB, .ACCDB ", 2)
                    'delete record
                ...
                If dts.Rows(i)("Deadline").ToString.Trim.StartsWith(DateToString(Now()).Substring(0, 10)) AndAlso dts.Rows(i)("Status").ToString.Trim = "downloaded" Then
                    ...
                    emailbody = "Scheduled download completed from the url: " & dts.Rows(i)("URL").ToString & " | See it in your  Downloads folder | Feel free to contact us at " & cntus & " if you have any questions. | OUReports"
                    ...
                    For j = 0 To emails.Length - 1
                        If emails(j).Trim <> "" Then
                            ...
                            If Not ret.StartsWith("ERROR!! ") Then
                                ...
                                sqlu = "UPDATE ourscheduleddownloads SET [Status]='email sent' WHERE ID=" & dts.Rows(i)("ID")
                                ret = ExequteSQLquery(sqlu)
                            End If
                        End If
                    Next
