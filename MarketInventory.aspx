<%@ Page Language="VB" AutoEventWireup="false" CodeFile="MarketInventory.aspx.vb" Inherits="MarketInventory" %>
<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>Market Inventory Model</title>
    <style type="text/css">
...
&nbsp;&nbsp;&nbsp;&nbsp;<asp:HyperLink ID="HyperLinkHelp" runat="server" NavigateUrl="DataAIHelp.aspx?hilt=Market%20Inventory" Target="_blank" CssClass="NodeStyle" Font-Names="Arial" ToolTip="Open help for market pages.">Help</asp:HyperLink>
...
<br /><br />
<div class="marketTitleBlock"><asp:Label ID="lblHeader" runat="server" Font-Size="22px" Font-Names="Arial" ToolTip="Build market and business model analysis using the selected report fields.">Market Inventory Model</asp:Label><asp:Label ID="LabelMarketSubtitle" runat="server" CssClass="marketSubtitle" ToolTip="This text explains the market model used by this page." Text="Shows inventory movement and reorder review. Primary Field can combine multiple fields; Value Field is summed for movement and velocity. Current Inventory field and Assumption % safety stock calculate supply periods, reorder point, reorder need, and inventory notes."></asp:Label></div>
<br />
...
<tr><td class="fieldLabel"><asp:Panel ID="pnlDateLabel" runat="server"><span title="Optionally select the date field used for activity, churn, retention, or time-aware market calculations.">Date field:</span></asp:Panel></td><td class="fieldControl"><asp:Panel ID="pnlDateControl" runat="server"><asp:DropDownList ID="DropDownDateField" runat="server" ToolTip="Optional date field for models that use activity dates."></asp:DropDownList></asp:Panel></td><td class="fieldLabel"><asp:Panel ID="pnlDateAggregationLabel" runat="server"><span title="Select how dates are grouped in period-based market results.">Date aggregation:</span></asp:Panel></td><td class="fieldControl"><asp:Panel ID="pnlDateAggregationControl" runat="server"><asp:DropDownList ID="DropDownDateAggregation" runat="server" ToolTip="Group date-based results by day, week, month, quarter, or year."><asp:ListItem Value="Day">Day</asp:ListItem><asp:ListItem Value="Week">Week</asp:ListItem><asp:ListItem Value="Month" Selected="True">Month</asp:ListItem><asp:ListItem Value="Quarter">Quarter</asp:ListItem><asp:ListItem Value="Year">Year</asp:ListItem></asp:DropDownList></asp:Panel><asp:Panel ID="pnlSecondaryControl" runat="server" Visible="False"><asp:DropDownList ID="DropDownSecondaryField" runat="server" ToolTip="Optional second field such as order, customer, or quantity."></asp:DropDownList></asp:Panel><asp:Panel ID="pnlSecondaryLabel" runat="server" Visible="False"></asp:Panel></td></tr>
<tr><td class="fieldLabel" title="Enter the safety-stock percentage used to calculate the reorder point.">Assumption %:</td><td class="fieldControl"><asp:TextBox ID="txtAssumption" runat="server" Width="70px" ToolTip="Safety-stock percentage added to velocity for reorder point calculation.">10</asp:TextBox></td><td class="fieldLabel" title="Select the numeric field that contains current stock, inventory on hand, available quantity, or balance.">Current Inventory field:</td><td class="fieldControl"><asp:DropDownList ID="DropDownInventoryField" runat="server" ToolTip="Current stock, inventory on hand, available quantity, or balance field used for Supply Periods and Reorder Needed."></asp:DropDownList></td></tr>
