<%@ Page Language="VB" AutoEventWireup="false" CodeFile="MarketElasticity.aspx.vb" Inherits="MarketElasticity" %>
<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>Market Pricing Elasticity Model</title>
    <style type="text/css">
...
<Nodes>
                                        ...
                                        <asp:TreeNode Text="Market Dashboard" Value="MarketAdmin.aspx" NavigateUrl="MarketAdmin.aspx" Expanded="True">
                                            ...
                                            <asp:TreeNode Text="Market Elasticity" Value="MarketElasticity.aspx" NavigateUrl="MarketElasticity.aspx"></asp:TreeNode>
...
&nbsp;&nbsp;&nbsp;&nbsp;<asp:HyperLink ID="HyperLinkHelp" runat="server" NavigateUrl="DataAIHelp.aspx?hilt=Market%20Elasticity" Target="_blank" CssClass="NodeStyle" Font-Names="Arial" ToolTip="Open help for market pages.">Help</asp:HyperLink>
...
<br /><br />
<div class="marketTitleBlock"><asp:Label ID="lblHeader" runat="server" Font-Size="22px" Font-Names="Arial" ToolTip="Build market and business model analysis using the selected report fields.">Market Pricing Elasticity Model</asp:Label><asp:Label ID="LabelMarketSubtitle" runat="server" CssClass="marketSubtitle" ToolTip="This text explains the market model used by this page." Text="Shows how quantity changes as price changes. Primary Field can combine multiple fields; Price Field is grouped into price bands, Quantity Field is compared between bands, and Assumption % projects price, quantity, revenue, and revenue impact from elasticity."></asp:Label></div>
<br />
<table class="controlpanel" cellpadding="4" cellspacing="0">
<tr><td class="fieldLabel" title="Select one or more category, product, customer, location, or other fields used to group the elasticity model.">Primary field(s):</td><td class="fieldControl"><asp:Panel ID="pnlPrimarySingle" runat="server"><asp:DropDownList ID="DropDownDimension" runat="server" ToolTip="Product, category, region, customer, or other market dimension."></asp:DropDownList></asp:Panel><asp:Panel ID="pnlPrimaryMulti" runat="server" Visible="False"><asp:ListBox ID="ListBoxDimension" runat="server" SelectionMode="Multiple" Rows="4" Width="210px" ToolTip="Select one or more fields to combine as the primary market grouping."></asp:ListBox></asp:Panel></td><td class="fieldLabel" title="Select the price field used to build price bands.">Price field:</td><td class="fieldControl"><asp:DropDownList ID="DropDownValueField" runat="server" ToolTip="Price, unit price, or price-per-unit field."></asp:DropDownList></td></tr>
<tr><td class="fieldLabel"><asp:Panel ID="pnlDateLabel" runat="server"><span title="Optional date field, hidden when it is not used by the current model.">Date field:</span></asp:Panel></td><td class="fieldControl"><asp:Panel ID="pnlDateControl" runat="server"><asp:DropDownList ID="DropDownDateField" runat="server" ToolTip="Optional date field for models that use activity dates."></asp:DropDownList></asp:Panel></td><td class="fieldLabel"><asp:Panel ID="pnlSecondaryLabel" runat="server"><span title="Select quantity, units, or sales volume used to compare demand across price bands.">Quantity field:</span></asp:Panel></td><td class="fieldControl"><asp:Panel ID="pnlSecondaryControl" runat="server"><asp:DropDownList ID="DropDownSecondaryField" runat="server" ToolTip="Quantity, units, or sales-volume field used for elasticity."></asp:DropDownList></asp:Panel><asp:Panel ID="pnlDateAggregationLabel" runat="server" Visible="False"></asp:Panel><asp:Panel ID="pnlDateAggregationControl" runat="server" Visible="False"><asp:DropDownList ID="DropDownDateAggregation" runat="server"><asp:ListItem Value="Month" Selected="True">Month</asp:ListItem></asp:DropDownList></asp:Panel></td></tr>
<tr><td class="fieldLabel" title="Enter a percentage assumption used by scenario or business model calculations.">Assumption %:</td><td class="fieldControl"><asp:TextBox ID="txtAssumption" runat="server" Width="70px" ToolTip="Optional assumption kept for shared market exports and AI interpretation.">10</asp:TextBox></td><td colspan="2"></td></tr>
<tr><td align="left" colspan="4" style="font-weight:bold;"><span title="Filter the market model result grid by text.">Search:</span> <asp:TextBox ID="txtSearch" runat="server" Width="260px" ToolTip="Type text to filter the market model results shown in the grid."></asp:TextBox><asp:Button ID="ButtonBuild" runat="server" CssClass="ticketbutton" Text="Build" OnClick="ButtonBuild_Click" ToolTip="Build the elasticity model using the selected fields." /><asp:Button ID="ButtonReset" runat="server" CssClass="ticketbutton" Text="Reset" OnClick="ButtonReset_Click" ToolTip="Clear search and restore default market model assumptions." /><asp:Button ID="ButtonExportCSV" runat="server" CssClass="ticketbutton" Text="CSV" OnClick="ButtonExportCSV_Click" ToolTip="Export the current elasticity results to CSV." /><asp:Button ID="ButtonExportExcel" runat="server" CssClass="ticketbutton" Text="Excel" OnClick="ButtonExportExcel_Click" ToolTip="Export the current elasticity results to Excel." /><asp:LinkButton OnClientClick="return showWaitingPanel();" ID="lnkMarketAI" runat="server" CssClass="aiLinkButton" Font-Names="Arial" OnClick="lnkMarketAI_Click" ToolTip="Ask AI to interpret the current elasticity results, opportunities, risks, and recommended actions.">AI</asp:LinkButton></td></tr>
</table>
...
<div style="overflow:auto; max-width:100%;"><asp:GridView ID="GridViewMarket" runat="server" CssClass="marketgrid" AutoGenerateColumns="True" GridLines="Both" ToolTip="Market elasticity results. Click record counts to open the corresponding records in Data Explorer."></asp:GridView></div>
